# Milvus 3.0 与 Snowflake 的共识：数据引力正在重写检索架构

## 元信息

- 来源链接: https://mp.weixin.qq.com/s/fC0NTG2oaejTPuj3JgrQfQ
- 公众号: 王人书
- 作者: Alden
- 发布时间: 2026年8月4日 08:08
- 题图: https://mmbiz.qpic.cn/mmbiz_jpg/7p2YcxeRCWchFeOv9NZB7BxIoHNJzJbyVXWFJmDyaXCTcjp1xO1dE1bWbowrrgAvOmuo1bRUZHFdsmAJfscyolNffbqApy9r8JwJ2iavHxqQ/0?wx_fmt=jpeg
- 摘要: 从数据引力出发，拆解 Milvus 3.0 的 Vector Lakebase 如何通过外部集合、Loon、快照、Spark 和引擎下推，把向量检索放回数据所在的位置。

## 正文

大家好，我是 Alden，LLM 开发工程师。这是 [LLM 工程](https://mp.weixin.qq.com/mp/appmsgalbum?__biz=MzA3NDE2ODIwMw==&action=getalbum&album_id=4269490557774643212#wechat_redirect) 的第 9 篇。Milvus 3.0 的 Vector Lakebase 追问的是更难的问题：数据已经在湖里，在线检索怎样不靠另一份副本，也能在延迟、一致性和治理上站得住。

字数 4000，预计阅读 **14 分钟。**建议先转后看。

![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/7p2YcxeRCWf5TX9prbtDYVyQ6BLLKWgpJrBKUWPCq3f5JHcuacYVFkia7mMt1HQKvXXP555aaNpSqcTCn2vkqZpkNmG1QVCYmF51QyMdJNtk/640?wx_fmt=png&from=appmsg#imgIndex=0)

[Milvus 3.0 发布，](https://mp.weixin.qq.com/s?__biz=MzUzMDI5OTA5NQ==&mid=2247513252&idx=1&sn=eef2891c3416f2a15dee661f36c46e0a&scene=21#wechat_redirect)Vector Lakebase 把向量检索的底座换了一种组织方式：湖原生数据、向量索引和检索计算服务同一份数据源。数据仍留在开放表格式和对象存储中，Milvus 则在数据所在的位置建立和服务检索。

**Data Gravity** 是 Snowflake 企业 AI 架构里的一个核心判断：数据规模、治理规则、血缘和权限一旦形成重心，系统拓扑就会向数据收拢。Milvus 3.0 把这一约束带到向量检索，处理的是 Embedding 已经在湖里后，ANN、回表、批处理和模型迭代如何不再围绕副本运转。Vector Lakebase 将答案拆到四层：

- **数据归属，External Collections 直接链接湖表，数据源保持统一，不再以检索副本为中心。**
- **随机读取，Loon 按点查路径组织数据，让湖原生存储可以承担 ANN 后的回表。**
- **版本一致性，Snapshot 和 Spark Connector 让训练、评估与线上检索共享版本边界。**
- **候选集计算，排序、聚合、分面与结构化多向量进入引擎，候选集不用反复离开数据路径。**

这四层按顺序连在一起。数据留在湖中是起点，读路径、时间边界和计算位置共同决定它能否成为生产级检索底座。

![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/BDbj75fnJWeIoodYAaLsroY3rF7c1XjiaarfTZNmoicEwlEFhWiaGicicjTWuACQ7FyibQDPwsa23MicliapqBeyLIcX86zluXZE4mhicKRTciaIyVtIs/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=13)
...
这里的重点在于**数据拓扑。**传统路径把数据湖和向量库变成两个需要持续对齐的中心，Vector Lakebase 让湖表保留权威性，向量检索成为叠加在其上的能力。开放格式也让这条路径不依赖单一存储形态，索引机制可以面对 Lance、Parquet、Iceberg、Vortex 等格式。数据引力不再被误解为“把所有东西收进一个封闭系统”，而是让计算在不夺取数据所有权的前提下靠近数据。
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/BDbj75fnJWfNNuPRUnD6v6Hr082wa8bf8MTFSS8eIWbGeAsf0bUekPBnqAQkP2TFJTgPQjic1XHibcvuxqHalt3lYnxG3oiaXQLjSfNiboib4Ob8/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=2)
...
Vector Lakebase 的辨识度也在这里。答案未必总是湖原生，也未必总是原生 Collection；但当数据规模、治理和反馈闭环进入系统主路径，围绕副本堆叠服务的架构会越来越难维护。Milvus 3.0 提供了一种值得认真评估的向量检索底座形态。

### 写在最后

过去，数据平台保存事实，检索系统检索事实，模型生成表达，导出任务、接口和缓存把它们松散连接。RAG 与 Agent 进入业务主链路后，一次回答同时依赖正确版本、权限口径和可回流的反馈。数据已经是推理运行时的一部分。

Vector Lakebase 重划了这条边界：检索计算向受治理的数据靠拢，避免下游重复维护状态、权限与时间线。它缩短同步距离，也把数据质量、元数据与评估方法直接纳入检索质量。向量检索服务的，从来不只是相似度，而是组织此刻对哪些信息可信、可见、可用的判断。

![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/7p2YcxeRCWenHPu9aHOR6mcls5CicfezD1koQa065Lo7tuTPcODNx2yKvebzrKlvUnofgsYAViafGm3zxiblqjMZVOzGicFZDY4p0DNmWpwsyQM/640?from=appmsg#imgIndex=3)

### 关联阅读

[一份让 GPT-5.6 Sol Ultra 挑战 50 年猜想的 Prompt，到底设计了什么？](https://mp.weixin.qq.com/s?__biz=MzA3NDE2ODIwMw==&mid=2247487910&idx=1&sn=ce707a3fa74364981c1acac70c039e5d&scene=21#wechat_redirect)

[Harness + TDD：后 Vibe Coding 时代，工程能力正重新被看见](https://mp.weixin.qq.com/s?__biz=MzA3NDE2ODIwMw==&mid=2247487871&idx=1&sn=2210cf6c00055c2a441bf2238cb7ca71&scene=21#wechat_redirect)
...
[Harness engineering：在智能体优先世界中发挥 Codex 的力量](https://mp.weixin.qq.com/s?__biz=MzA3NDE2ODIwMw==&mid=2247487871&idx=1&sn=2210cf6c00055c2a441bf2238cb7ca71&scene=21#wechat_redirect)
...
- [LLM 工程](https://mp.weixin.qq.com/mp/appmsgalbum?__biz=MzA3NDE2ODIwMw==&action=getalbum&album_id=4269490557774643212#wechat_redirect)
- [Milvus 3.0 发布，](https://mp.weixin.qq.com/s?__biz=MzUzMDI5OTA5NQ==&mid=2247513252&idx=1&sn=eef2891c3416f2a15dee661f36c46e0a&scene=21#wechat_redirect)
- [一份让 GPT-5.6 Sol Ultra 挑战 50 年猜想的 Prompt，到底设计了什么？](https://mp.weixin.qq.com/s?__biz=MzA3NDE2ODIwMw==&mid=2247487910&idx=1&sn=ce707a3fa74364981c1acac70c039e5d&scene=21#wechat_redirect)
- [Harness engineering：在智能体优先世界中发挥 Codex 的力量](https://mp.weixin.qq.com/s?__biz=MzA3NDE2ODIwMw==&mid=2247487871&idx=1&sn=2210cf6c00055c2a441bf2238cb7ca71&scene=21#wechat_redirect)
- [熵增、刘慈欣与 LLM：声明式架构是对抗混乱的唯一解吗？](https://mp.weixin.qq.com/s?__biz=MzA3NDE2ODIwMw==&mid=2247487646&idx=1&sn=a88b7e1416532499d6d6115bf8fb5f10&scene=21#wechat_redirect)
- [被 Claude 吞噬的应用层：从 MCP、Skills 到 PTC 的跨维度打击](https://mp.weixin.qq.com/s?__biz=MzA3NDE2ODIwMw==&mid=2247487632&idx=1&sn=3e8558250b98c6dda4d38a8213d83ee4&scene=21#wechat_redirect)
